...
try:
    cbp = open('repo_language.bp')
    tasks = json.load(cbp)
except:
    cbp = open('repo_language.bp','w')
    tasks = []
    json.dump(tasks,cbp)
finally:
    cbp.close()
...
logging.basicConfig(filename='repo_language.log',level=logging.WARNING,
format='%(asctime)s %(filename)s %(levelname)s %(threadName)s %(message)s')


def saving(repo,languages,tasks):
    with open('repo_language.json','a') as cj:
        cj.write(repo+'\t')
        json.dump(languages,cj)
        cj.write('\n')
    tasks.remove(repo)
    with open('repo_language.bp','w') as cbp:
        json.dump(tasks,cbp)
...
def main(repo,auth_account,proc_cnt):
    ...
    for i in range(5):
        try:
            r = requests.get('https://api.github.com/repos/'+repo+"/languages",auth=auth_account)
            break
    ...
    if int(r.headers['X-RateLimit-Remaining']) <= proc_cnt:
        ...
        while time.time() < int(r.headers['X-RateLimit-Reset']):
            time.sleep(300)
    if 'message' in j:
        logging.warning(j['message']+'\turl:https://api.github.com/repos/'+repo+"/languages")
        return []
    return j
...
def consumer(task_queue,tasks,result_queue,auth_account,proc_name,proc_cnt):
    ...
    while True:
        ...
        languages = main(repo,auth_account,proc_cnt)
        task_queue.task_done() # Must be called the same times as .get()
        result_queue.put([repo,languages])
...
while True:
    try:
        repo,languages = result_queue.get(timeout=60)
    except:
        break
    saving(repo,languages,tasks)
